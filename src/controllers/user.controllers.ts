...
export const Register = expressAsyncHandler(
	async (
		...
		res.status(202).json({
			...
			links: {
				self: 'https://mkk-book-store.herokuapp.com/api/v1/users/register',
				users: 'https://mkk-book-store.herokuapp.com/api/v1/users',
				books: 'https://mkk-book-store.herokuapp.com/api/v1/books',
			},
		});
...
export const Login = expressAsyncHandler(
	async (
		...
		try {
			...
			if (!user)
				return next(
					createHttpError(404, 'Incorrect email or password'),
				);
			...
			if (!isMatchPwd)
				return next(
					createHttpError(403, 'Incorrect email or password'),
				);
